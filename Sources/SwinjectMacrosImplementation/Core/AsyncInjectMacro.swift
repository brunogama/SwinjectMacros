    ...
    private static func createAsyncAccessorMethod(
        ...
        return """
            ...
            let task = Task<\(propertyType), Error> {
                    ...
                    do {
                        ...
                        let resolved = try await withThrowingTaskGroup(of: \(propertyType)?.self) { group in
                            ...
                            group.addTask {
                                ...
                                throw AsyncInjectionError.resolutionTimeout(type: "\(propertyType)", timeout: \(
                                    config
                                        .timeout
        ))
                            }
                        ...
                        guard let service = resolved else {
                            throw AsyncInjectionError.serviceNotRegistered(serviceName: \(
                                config.serviceName
                                    .map { "\"\($0)\"" } ?? "nil"
        ), type: "\(propertyType)")
                        }
                ...
                let endTime = Date()
                let duration = endTime.timeIntervalSince(startTime)
                let finalError = lastError ?? AsyncInjectionError.maxRetriesExceeded(type: "\(
                    propertyType
