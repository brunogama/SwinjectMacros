...
public struct InjectableMacro: MemberMacro, ExtensionMacro {
    ...
    private static func generateRegistrationMethod(
        ...
        let dependencies = serviceInfo.dependencies.filter { dep in
            dep.classification == InjectableParameterClassification.serviceDependency || dep
                .classification == InjectableParameterClassification.protocolDependency
        }

        // Build parameter list for initializer call
        let parameterList = dependencies.map { dependency in
            if dependency.isOptional {
                // For optional types, remove the ? suffix before resolving
                let typeWithoutOptional = dependency.type.hasSuffix("?")
                    ? String(dependency.type.dropLast())
                    : dependency.type
                return "\(dependency.name): resolver.synchronizedResolve(\(typeWithoutOptional).self)"
            } else {
...
private struct InjectableMacroError: DiagnosticMessage {
    let message: String
    let diagnosticID = MessageID(domain: "SwinjectUtilityMacros", id: "InjectableError")
    let severity: DiagnosticSeverity = .error
}

private struct InjectableMacroWarning: DiagnosticMessage {
    let message: String
    let diagnosticID = MessageID(domain: "SwinjectUtilityMacros", id: "InjectableWarning")
