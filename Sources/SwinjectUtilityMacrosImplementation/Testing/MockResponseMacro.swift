...
public struct MockResponseMacro: PeerMacro {
    ...
    private static func generateMockResponseStorage(
        ...
        let behaviorType = generateBehaviorType(
            parameters: parameters,
            returnType: returnType,
            isAsync: isAsync,
            isThrows: isThrows
    ...
    private static func generateMockResponseMethods(
        ...
        let setBehaviorMethod = generateSetBehaviorMethod(
            methodName: methodName,
            parameters: parameters,
            returnType: returnType
        )
    ...
    private static func generateSetBehaviorMethod(
        ...
        if paramTypes.isEmpty {
            ...
            func set\(
                methodName
                    .capitalized
            )MockBehavior(_ behavior: @escaping () async throws -> \(returnTypeString)) {
                \(methodName)MockBehavior = behavior
            }
            """
        } else {
            return """
            func set\(methodName.capitalized)MockBehavior(_ behavior: @escaping (\(
                paramTypes
                    .joined(separator: ", ")
            )) async throws -> \(returnTypeString)) {
                \(methodName)MockBehavior = behavior
            }
            """
        }
