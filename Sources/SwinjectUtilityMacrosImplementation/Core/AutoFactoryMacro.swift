...
public struct AutoFactoryMacro: PeerMacro, MemberMacro {
    ...
    public static func expansion(
        ...
        // For now, we don't add any members to the original type
        []
    }
    ...
    private static func extractAutoFactoryServiceInfo(
        ...
        if runtimeParameters.isEmpty {
            let diagnostic = Diagnostic(
                ...
                message: AutoFactoryMacroWarning(
                    message: "@AutoFactory is designed for services with runtime parameters. Consider using @Injectable instead."
                )
            )
            context.diagnose(diagnostic)
        }
    ...
    private static func separateDependencies(_ dependencies: [AutoFactoryDependencyInfo])
        -> (injected: [AutoFactoryDependencyInfo], runtime: [AutoFactoryDependencyInfo])
    {
    ...
    private static func classifyParameter(_ param: FunctionParameterSyntax) -> AutoFactoryParameterClassification {
        ...
        // Collection types with value type elements are runtime parameters
        if typeText.hasPrefix("[") && typeText.hasSuffix("]") {
            // Extract element type from array syntax
            let elementType = String(typeText.dropFirst().dropLast()).trimmingCharacters(in: .whitespaces)
            if valueTypes.contains(where: { elementType.hasPrefix($0) }) {
                return .runtimeParameter
            }
        }

        // Dictionary types with value type elements are runtime parameters
        if typeText
            .hasPrefix("Dictionary<") ||
            (typeText.contains(":") && typeText.hasPrefix("[") && typeText.hasSuffix("]"))
        {
            return .runtimeParameter
        }

        // Set types with value type elements are runtime parameters
        if typeText.hasPrefix("Set<") {
            return .runtimeParameter
        }
    ...
    private static func generateFactoryImplementation(
        ...
        let injectedParams = serviceInfo.injectedDependencies.map { dependency in
            generateContainerResolverCall(for: dependency)
        }
        ...
        class \(implName): \(factoryName) {
            ...
            \(methodSignature) {
                return \((config.canThrow || serviceInfo.canThrow) ? "try " :
            ""
        )\((config.isAsync || serviceInfo.isAsync) ? "await " : "")\(serviceName)(
                    \(initParams)
                )
            }
        }
    ...
    private static func generateResolverCall(for dependency: AutoFactoryDependencyInfo) -> String {
        if dependency.isOptional {
            "\(dependency.name): resolver.synchronizedResolve(\(dependency.type).self)"
        } else {
            "\(dependency.name): resolver.synchronizedResolve(\(dependency.type).self)!"
        }
    }

    private static func generateContainerResolverCall(for dependency: AutoFactoryDependencyInfo) -> String {
        if dependency.isOptional {
            "\(dependency.name): container.synchronizedResolve(\(dependency.type).self)"
        } else {
            "\(dependency.name): container.synchronizedResolve(\(dependency.type).self)!"
        }
    }
...
private struct AutoFactoryMacroError: DiagnosticMessage {
    let message: String
    let diagnosticID = MessageID(domain: "SwinjectUtilityMacros", id: "AutoFactoryError")
    let severity: DiagnosticSeverity = .error
}

private struct AutoFactoryMacroWarning: DiagnosticMessage {
    let message: String
    let diagnosticID = MessageID(domain: "SwinjectUtilityMacros", id: "AutoFactoryWarning")
