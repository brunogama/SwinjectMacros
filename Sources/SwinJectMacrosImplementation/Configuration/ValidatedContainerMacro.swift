...
public struct ValidatedContainerMacro: PeerMacro {
    ...
    private static func generateValidationMethods(
        ...
        static func validateContainer(_ container: Container) throws {
            \(analysis.serviceRegistrations.map { "try validate\($0.serviceType.replacingOccurrences(of: ".", with: ""))(container)" }.joined(separator: "\n        "))
        }
        """
    ...
    private static func generateDependencyGraphMethods(
        ...
        static func getDependencyGraph() -> DependencyGraph {
            let services: Set<String> = [\(analysis.serviceRegistrations.map { "\"\($0.serviceType)\"" }.joined(separator: ", "))]
    ...
    private static func generateDocumentationMethods(
        ...
        static func getServiceDocumentation() -> [String: String] {
            return [
                \(analysis.serviceRegistrations.map { "\"\($0.serviceType)\": \"Service registration for \($0.serviceType)\"" }.joined(separator: ",\n                "))
            ]
        }
        """
...
private enum ValidationWarning {
    ...
    var description: String {
        ...
        case .invalidScope(let issue):
            return "Scope configuration issue: \(issue)"
        case .performanceIssue(let issue):
            return "Performance concern: \(issue)"
        }
    }
}
...
private struct ValidatedContainerMacroError: DiagnosticMessage, Error {
    let message: String
    let diagnosticID = MessageID(domain: "SwinJectMacros", id: "ValidatedContainerError")
    let severity: DiagnosticSeverity = .error
}

private struct ValidatedContainerMacroWarning: DiagnosticMessage {
    let message: String
    let diagnosticID = MessageID(domain: "SwinJectMacros", id: "ValidatedContainerWarning")
