...
class ContentViewModel: ContentViewModelProtocol {
    ...
    func loadInitialData() async {
        ...
        do {
            ...
            let (_, _, _) = try await (usersResult, analyticsResult, authResult)
    ...
    func refreshData() async {
        ...
        do {
            ...
            let (_, _) = try await (usersRefresh, analyticsRefresh)
    ...
    func trackScreenView() {
        Task {
            await analyticsService.trackScreenView("content_main", userId: getCurrentUserId())
        }
    }
    ...
    private func getCurrentUserId() -> String? {
        ...
        return isAuthenticated ? "current_user" : nil
    }
}
    ...
    func deleteUser(_ userId: String) async {
        ...
        do {
            ...
            if success {
                ...
                throw UserError.deletionFailed(NSError(domain: "UserDeletion", code: 0, userInfo: [NSLocalizedDescriptionKey: "Deletion returned false"]))
            }
...
extension ContentViewModel {
    ...
    private func handleError(_ error: Error, operation: String) async {
        ...
        case let userError as UserError:
            userFriendlyMessage = userError.localizedDescription
        case let authError as AuthError:
            userFriendlyMessage = authError.localizedDescription
        case let networkError as NetworkError:
            userFriendlyMessage = "Network issue: \(networkError.localizedDescription)"
        case let analyticsError as AnalyticsError:
            userFriendlyMessage = "Analytics issue: \(analyticsError.localizedDescription)"
        default:
